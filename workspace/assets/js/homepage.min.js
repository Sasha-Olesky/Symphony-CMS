/*  
	-----------------------------------------------------------------
	Section description: Compiled Vendors JS.
	...
	MINIFIED BY Jo @ Orchard. 
	Note AudioPlayer has been taken out as it is not used in this build
	-----------------------------------------------------------------
*/ 
...
*/
var ratio,ratioDragger,iScroll,iScroll2,iPosition,dynamicScrollWidth;$.tiny=$.tiny||{};$.tiny.scrollbar={options:{axis:'x',wheel:20,mouseWheel:'1',size:'auto',draggerWidth:'auto'}};$.fn.tinyscrollbar=function(options){var options=$.extend({},$.tiny.scrollbar.options,options);this.each(function(){$(this).data('tsb',new Scrollbar($(this),options))});return this};$.fn.tinyscrollbar_update=function(sScroll){return $(this).data('tsb').update(sScroll)};function Scrollbar(root,options){var oSelf=this;var oWrapper=root;var oViewport={obj:$('.viewport',root)};var oImages={obj:$('.images',root)};var oMilestones={obj:$('.milestones',root)};var oContent={obj:$('.content',root)};var oScrollbar={obj:$('.scrollbar',root)};var oScrollbar2={obj:$('.scrollbar',root)};var oTrack={obj:$('.track',oScrollbar.obj)};var oDragger={obj:$('.dragger',oScrollbar.obj)};var sAxis=options.axis=='x',sDirection=sAxis?'left':'top',sSize=sAxis?'Width':'Height';iScroll,iScroll2,iPosition={start:-30,now:-30},iMouse={};function initialize(){oSelf.update();setEvents();return oSelf}
this.update=function(sScroll){oViewport[options.axis]=oViewport.obj[0]['offset'+sSize];oMilestones[options.axis]=oMilestones.obj[0]['offset'+sSize];oImages[options.axis]=oImages.obj[0]['scroll'+sSize];oContent[options.axis]=oContent.obj[0]['scroll'+sSize];oImages.ratio=oViewport[options.axis]/oImages[options.axis];oContent.ratio=oMilestones[options.axis]/oContent[options.axis];oTrack[options.axis]=options.size=='auto'?oViewport[options.axis]:dynamicScrollWidth;oDragger[options.axis]=Math.min(oTrack[options.axis],Math.max(0,(options.draggerWidth=='auto'?(oTrack[options.axis]*oImages.ratio):options.draggerWidth)));oScrollbar.ratio=options.draggerWidth=='auto'?(oImages[options.axis]/oTrack[options.axis]):(oImages[options.axis]-oViewport[options.axis])/(oTrack[options.axis]-oDragger[options.axis]);iScroll=(sScroll=='relative'&&oImages.ratio<=1)?Math.min((oImages[options.axis]-oViewport[options.axis]),Math.max(0,iScroll)):0;iScroll=(sScroll=='bottom'&&oImages.ratio<=1)?(oImages[options.axis]-oViewport[options.axis]):isNaN(parseInt(sScroll))?iScroll:parseInt(sScroll);oScrollbar2.ratio=options.draggerWidth=='auto'?(oContent[options.axis]/oTrack[options.axis]):(oContent[options.axis]-oMilestones[options.axis])/(oTrack[options.axis]-oDragger[options.axis]);iScroll2=(sScroll=='relative'&&oContent.ratio<=1)?Math.min((oContent[options.axis]-oMilestones[options.axis]),Math.max(0,iScroll2)):0;iScroll2=(sScroll=='bottom'&&oContent.ratio<=1)?(oContent[options.axis]-oMilestones[options.axis]):isNaN(parseInt(sScroll))?iScroll2:parseInt(sScroll);if(oScrollbar.ratio<0&&oScrollbar2.ratio<0)$('.dragger').hide();else $('.dragger').show();ratio=oScrollbar2.ratio/oScrollbar.ratio;ratioDragger=(oTrack[options.axis]-oDragger[options.axis])/(oImages[options.axis]-oTrack[options.axis]);setSize()};function setSize(){oDragger.obj.css(sDirection,iScroll/oScrollbar.ratio);oImages.obj.css(sDirection,-iScroll);oContent.obj.css(sDirection,-iScroll2);iMouse.start=oDragger.obj.offset()[sDirection];var sCssSize=sSize.toLowerCase();oScrollbar.obj.css(sCssSize,oTrack[options.axis]);oTrack.obj.css(sCssSize,oTrack[options.axis]);oDragger.obj.css(sCssSize,oDragger[options.axis])};function setEvents(){oDragger.obj.bind('mousedown',start);oDragger.obj[0].ontouchstart=function(oEvent){oEvent.preventDefault();oDragger.obj.unbind('mousedown');start(oEvent.touches[0]);return!1};oTrack.obj.bind('mouseup',drag);if(options.mouseWheel=='1'&&this.addEventListener){oWrapper[0].addEventListener('DOMMouseScroll',wheel,!1);oWrapper[0].addEventListener('mousewheel',wheel,!1)}
else if(options.mouseWheel=='1'){oWrapper[0].onmousewheel=wheel}};function start(oEvent){iMouse.start=sAxis?oEvent.pageX:oEvent.pageY;var oDraggerDir=parseInt(oDragger.obj.css(sDirection));iPosition.start=oDraggerDir=='auto'?0:oDraggerDir;$(document).bind('mousemove',drag);document.ontouchmove=function(oEvent){$(document).unbind('mousemove');drag(oEvent.touches[0])};$(document).bind('mouseup',end);oDragger.obj.bind('mouseup',end);oDragger.obj[0].ontouchend=document.ontouchend=function(oEvent){$(document).unbind('mouseup');oDragger.obj.unbind('mouseup');end(oEvent.touches[0])};return!1};function wheel(oEvent){if(!(oImages.ratio>=1)||!(oContent.ratio>=1)){var oEvent=oEvent||window.event;var iDelta=oEvent.wheelDelta?oEvent.wheelDelta/120:-oEvent.detail/3;iScroll-=iDelta*options.wheel*oScrollbar.ratio;iScroll=Math.min((oImages[options.axis]-oViewport[options.axis]),Math.max(0,iScroll));iScroll2-=iDelta*options.wheel*oScrollbar2.ratio;iScroll2=Math.min((oContent[options.axis]-oMilestones[options.axis]),Math.max(0,iScroll2));oDragger.obj.css(sDirection,iScroll/oScrollbar.ratio);oImages.obj.css(sDirection,-iScroll);oContent.obj.css(sDirection,-iScroll2);oEvent=$.event.fix(oEvent);oEvent.preventDefault()}};function end(oEvent){$(document).unbind('mousemove',drag);$(document).unbind('mouseup',end);oDragger.obj.unbind('mouseup',end);document.ontouchmove=oDragger.obj[0].ontouchend=document.ontouchend=null;return!1};function drag(oEvent){if(!(oImages.ratio>=1)||!(oContent.ratio>=1)){iPosition.now=Math.min((oTrack[options.axis]-oDragger[options.axis]),Math.max(0,(iPosition.start+((sAxis?oEvent.pageX:oEvent.pageY)-iMouse.start))));iScroll=iPosition.now*oScrollbar.ratio;iScroll2=iPosition.now*oScrollbar2.ratio;if(!(oImages.ratio>=1))$(oImages.obj).stop(!0,!0).animate({left:-iScroll},1000,'easeInOutQuad');if(!(oContent.ratio>=1))$(oContent.obj).stop(!0,!0).animate({left:-iScroll2},700,'easeInOutQuad');$(oDragger.obj).stop(!0,!0).animate({left:iPosition.now},700,'easeInOutQuad')}
return!1};return initialize()}
...
return this};$.fn.tipsy.defaults={delayIn:0,delayOut:0,fade:!1,fallback:'',gravity:'n',html:!1,live:!1,offset:0,opacity:1,title:'title',trigger:'hover'};$.fn.tipsy.elementOptions=function(ele,options){return $.metadata?$.extend({},options,$(ele).metadata()):options}
...
/*  
	...
	With edits by Jo @ Orchard
	-------------------------------------------------------------
*/ 
(function($){$.fn.preload=function(fn){var len=this.length,i=0;return this.each(function(){var tmp=new Image,self=this;if(fn)tmp.onload=function(){fn.call(self,100*++i/len,i===len)};tmp.src=this.src})};$.fn.timelineSlider=function(options){var options=$.extend({timelineWidth:960,timelineHeight:500,upperAreaHeight:265,lowerAreaHeight:215,draggerHeight:21,audioPlayer:!0,audioAutoPlay:!0,audioLoop:!0,audioFilePath:'mp3/music.mp3',responsive:!0,marksOnMobile:!0,version:'dark',designStyle:'default',accentColor:'#299ec4',timelineBorderWidth:4,timelineBorderColor:'#ffffff',shadow:!0},options);return this.each(function(){var myWindow=$(window),windowWidth=myWindow.width(),wrapper=$(this),imagePath='http://rhydycarwest.com/workspace/assets/images',tl=wrapper.find('.timeline'),viewport=tl.find('.viewport'),images=tl.find('.viewport .images'),milestones=tl.find('.milestones'),content=tl.find('.milestones .content'),bar=tl.find('.scrollbar'),track=tl.find('.scrollbar .track'),dragger=tl.find('.scrollbar .track .dragger'),marks=tl.find('.marks'),vidBt=tl.find('.video_bt'),imgBt=tl.find('.image_bt,.video_bt'),readBt=tl.find('.readmore');wrapper.append('<div class="preload"></div>');$('img').preload(function(perc,done){if(done){var contentWidth=0,imagesWidth=0,draggerWidth=0;content.children().each(function(){contentWidth=contentWidth+$(this).outerWidth(!0)});images.find('.point').each(function(){imagesWidth=imagesWidth+$(this).outerWidth(!0)});draggerWidth=parseInt(dragger.css('width'));wrapper.css('width',options.timelineWidth);wrapper.css('height',options.timelineHeight);images.css('width',imagesWidth);viewport.css('height',options.upperAreaHeight);content.css('width',contentWidth+100);milestones.css('height',options.lowerAreaHeight);bar.css('top',options.upperAreaHeight-options.draggerHeight);track.css('height',options.draggerHeight+1);dragger.css('height',options.draggerHeight);$('.preload').fadeOut(500);tl.animate({opacity:1},1000,'easeOutQuad');if(options.version=='dark')wrapper.addClass('dark');if(options.designStyle=='flat'){wrapper.addClass('flat');var draggerURL=imagePath.concat('/','timeline/dragger.png');dragger.find('img').attr('src',draggerURL)}
if(options.shadow)wrapper.addClass('shadow');if(options.timelineBorderWidth>0){wrapper.css('border',options.timelineBorderWidth+'px solid '+options.timelineBorderColor);milestones.css('border-top',options.timelineBorderWidth+'px solid '+options.timelineBorderColor)}
$("a[data-rel^='prettyPhoto']").prettyPhoto({social_tools:!1,theme:'timeline-slider'});readBt.tipsy({gravity:'w',fade:!0,offset:5});imgBt.append('<span></span>').hover(function(){$(this).children('span').stop(!0,!0).fadeIn(400)},function(){$(this).children('span').stop(!0,!0).fadeOut(200)});var marksAmount=marks.find('div').length;for(var i=0;i<marksAmount;i++){current=marks.find('div:nth-child('+(i+1)+')');current.stop(!0,!0).delay(500).animate({'left':current.attr("data-xpos"),opacity:1},700+100*i,'easeOutQuad').show().tipsy({gravity:'s',fade:!0,offset:3,fallback:current.attr("data-label")})};startDrag(images);var dragStarted=!0;dynamicScrollWidth=options.timelineWidth;tl.tinyscrollbar({wheel:20,mouseWheel:0,size:dynamicScrollWidth,draggerWidth:draggerWidth});var draggerPos=$('.init').attr('data-xpos');var yourView=$(window).width();var initPointWidth=$('.initPoint').outerWidth(!0);if(initPointWidth>yourView){if($('.onScreen').length>0){var showMeData=$('.onScreen')[0].getBoundingClientRect();var margin=yourView-showMeData.width,margin=margin/2;var goLeftViewport=showMeData.left-margin}else{var showMeData=$('.initPoint')[0].getBoundingClientRect();var goLeftViewport=showMeData.left}}else{if($('.edgeScreen').length>0){var showMeData=$('.edgeScreen')[0].getBoundingClientRect();var goLeftViewport=showMeData.left}else{var showMeData=$('.initPoint')[0].getBoundingClientRect();var margin=yourView-showMeData.width,margin=margin/2;var goLeftViewport=showMeData.left-margin}}
var initPointData=$('.initMilestone')[0].getBoundingClientRect();var milestoneMargin=yourView-initPointData.width,milestoneMargin=milestoneMargin/2;var goLeftTimeline=initPointData.left-milestoneMargin;$('.dragger').animate({'left':draggerPos},700);$('.viewport .images').animate({'left':'-'+goLeftViewport},700);$('.milestones .content').animate({'left':'-'+goLeftTimeline},700);if(options.responsive||options.timelineWidth=='auto'){function manageFullwidth(){if(options.timelineWidth=='auto'){wrapperToFull()}else{if(options.responsive&&windowWidth<options.timelineWidth+25){wrapperToFull()}else{wrapper.css('width',options.timelineWidth);dynamicScrollWidth=options.timelineWidth;tl.tinyscrollbar_update()}}
if(!options.marksOnMobile&&windowWidth<768){marks.hide()}else{marks.show()}}
function wrapperToFull(){wrapper.css('width','auto');dynamicScrollWidth=parseInt(wrapper.css('width'));tl.tinyscrollbar_update()}
var resizeTimer;$(window).on('resize',function(e){clearTimeout(resizeTimer);resizeTimer=setTimeout(function(){windowWidth=myWindow.width();manageFullwidth()},100)});manageFullwidth()}}});function startDrag(i){var leftLimit=0;i.draggable({axis:"x",start:function(event,ui){if(ui.position!=undefined){leftLimit=ui.position.left}},drag:function(event,ui){leftLimit=ui.position.left;var rightLimit=i.width()-wrapper.width();if(ui.position.left<0&&ui.position.left*-1>rightLimit)leftLimit=rightLimit*-1;if(ui.position.left>0)leftLimit=0;ui.position.left=leftLimit;content.css('left',leftLimit*ratio);dragger.css('left',leftLimit*-ratioDragger);iScroll=-leftLimit;iScroll2=-(content.position().left)}});i.addClass('drag_icon')};function stopDrag(i){i.draggable('destroy');i.css('cursor','default')}})}}(jQuery))

/*
*********************************************************************************************************************
* SCRIPT TO LOAD TIMELINE
*/
/*
$(document).ready(function(){var fullHeight=($(window).width()<768)?480:660;var upperHeight=($(window).width()<768)?300:480;$("#mytimeline").timelineSlider({timelineWidth:'auto',timelineHeight:fullHeight,upperAreaHeight:upperHeight,lowerAreaHeight:180,draggerHeight:21,audioPlayer:!1,responsive:!0,marksOnMobile:!1,version:'light',designStyle:'flat',accentColor:'#50883D',timelineBorderWidth:0,timelineBorderColor:'#ffffff',shadow:!1,})})
*/
$(document).ready(function(){var fullHeight=($(window).width()<768)?480:560;var upperHeight=($(window).width()<768)?300:380;$("#mytimeline").timelineSlider({timelineWidth:'auto',timelineHeight:fullHeight,upperAreaHeight:upperHeight,lowerAreaHeight:180,draggerHeight:21,audioPlayer:!1,responsive:!0,marksOnMobile:!1,version:'light',designStyle:'flat',accentColor:'#50883D',timelineBorderWidth:0,timelineBorderColor:'#ffffff',shadow:!1,})})

/* -------------------------------------- END JQUERY TIMELINE ----------------------------------------*/

/*  
	-------------------------------------------------------------
	Description: rest of script for Rhydycar West homepage
	Author: Jo @ Orchard
	-------------------------------------------------------------
*/ 
/*
*********************************************************************************************************************
* SCRIPT FOR HOMEPAGE BLOCKS CONCERTINA
*/
$(document).ready(function(){$('.accBlock_body').css('display','none');$('.not-first-tab').css('display','none');var acc=document.getElementsByClassName('accBlock_head');var i;for(i=0;i<acc.length;i++){acc[i].addEventListener('click',function(){this.classList.toggle('active');var expand=this.getElementsByClassName('expand')[0];var panel=this.nextElementSibling;if(panel.style.display==='block'){panel.style.display='none';this.setAttribute('aria-expanded','false');panel.setAttribute('aria-hidden','true');expand.innerHTML='+';if(panel.getElementsByClassName('block-tabs').length>0){var buttons=panel.getElementsByClassName('block-tab-link');var bi;for(bi=0;bi<buttons.length;bi++){buttons[bi].setAttribute('aria-selected','false')}}}else{panel.style.display='block';this.setAttribute('aria-expanded','true');panel.setAttribute('aria-hidden','false');expand.innerHTML='-';if(panel.getElementsByClassName('block-tabs').length>0){var firstButton=panel.getElementsByClassName('block-tab-link')[0];firstButton.setAttribute('aria-selected','true')}}});acc[i].addEventListener('keypress',function(e){if(e.keyCode===32||e.key===' '||e.code==='Space'){this.click()}})}
if($(window).width()>550){$('.openFirst').click()}
$('.block-tab-link').on('click',function(){var id=$(this).attr('data-tab');var thisBlock=$(this).closest('.block-body');thisBlock.find('.block-tab-content').css('display','none');thisBlock.find('.block-tab-link').removeClass('active');thisBlock.find('.block-tab-link').attr('aria-selected','false');$(this).addClass('active');$(this).attr('aria-selected','true');$('#'+id).css('display','block')})})

/*
*********************************************************************************************************************
* SCRIPT FOR TEXT MODALS / LIGHTBOXES
*/
$(document).ready(function(){$('.lightbox-trigger').off().on('click',function(){var triggered='#'+$(this).attr('id')+'-lightbox';$(triggered).css('display','block');$('body').addClass('modal-open')});document.onkeydown=function(evt){evt=evt||window.event;if(evt.keyCode==27){if($('body').hasClass('modal-open')){$('.lightbox-container').fadeOut(400);$('body').removeClass('modal-open')}}};$('.lightbox-container').off().on('click',function(e){if($(e.target).hasClass('lightbox')||$(e.target).parents('.lightbox').length>0){if($(e.target).hasClass('lightbox-close')||$(e.target).parents('.lightbox-close').length>0){$('.lightbox-container').fadeOut(400);$('body').removeClass('modal-open')}}else{$('.lightbox-container').fadeOut(400);$('body').removeClass('modal-open')}})})
