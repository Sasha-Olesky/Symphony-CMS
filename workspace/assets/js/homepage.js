/*	
	...
	additions by Jo @ Orchard

	REQUIRES JQUERY < 3 DUE TO WINDOW.LOAD / DOCUMENT.READY ISSUES
...
(function($) {
		...
		function ProgressBar(){
		 ...
		 // If it's a vertical scroll
			if (skel.breakpoint('small').active){
				...
				// Set max scrollable area
					max = docHeight - winHeight;
					$progressBar.attr('max', max);

				// Progress value change on scroll
			...
			// Else is horizontal scroll
				...
				var winWidth = $('progress').width(),
					wrapWidth = 0,
					$progressBar = $('progress'), max, value;

			// Calculate wrapper width
				$wrapper.children('section').each(function() {
					wrapWidth += $(this).width();
				});

			// Set max scrollable area
				max = wrapWidth - winWidth;
				$progressBar.attr('max', max);

			// Progress value change on scroll
				$document.on('scroll', function(){
					value = $document.scrollLeft();
					$progressBar.attr('value', value);
				});
			};
		};

	// Set up page sizing function.
		...
		// Calculate section 2 width
				var sw = 0;

				$('#l2').children().each(function(){
					sw += $(this).width();
					console.log('sw', sw);
				})
	...
	// Set up function to calculate isVertical variable.
		function IsVertical(){
			if (skel.breakpoint('small').active)
				{ return true; }
			else
	...
	// Set up function for magic scroll.
		function scrollAnimate( isVertical ){

			console.log(isVertical);

			$controller = new ScrollMagic.Controller({ vertical: isVertical });
			...
			var s2Width = $('.s2').width();

				$parallaxTween = new TimelineMax ()
					.add([
						TweenMax.to('#l1', 1, {backgroundPosition:"-160% 25px", ease: Linear.easeNone}),
						TweenMax.to('#l3', 1, {backgroundPosition:"-70% 100%", ease: Linear.easeNone})
					]);
				...
				$cyclistScene = new ScrollMagic.Scene({
					triggerElement: '#cyclist'
				})
					.setPin('#cyclist')
					//.addIndicators({name: 'cyclist'})
					.addTo($controller);
			}

		};

	// Recalculate wrapper width after images have loaded
		$('#wrapper').imagesLoaded( function() {
		// images have loaded
			console.log('images have loaded');
			WrapperSize();

		// for each holding div, give it the width of it's child image
			$('.holder').each(function(i, e){
				var wi = $(this).children().width();
				console.log('wi ', wi);
				$(this).css('width', wi);
				$(this).children().css('width', wi);
			});
	...
	// Ready event.
		$(function() {
			...
			// Set class to add proper css properties for hiding scrollbars
				$bodyHtml.addClass('home');
			...
			// Page has loaded
				window.onload = function() {
					...
					// Set wrapper size & progress bar
						WrapperSize();
						ProgressBar();

					// Set variable based on viewport width
			...
			// Reset wrapper size & progress bar
				$window
					.on('resize', function() {

					// Set progress bar to be width of window
						ProgressBar();
					
					// Is the small viewport activated?
					...
					// If there has been a change in page orientation
			...
			// Tweaks/fixes.
				...
				// IE & Edge: set class in case of fixes needed
