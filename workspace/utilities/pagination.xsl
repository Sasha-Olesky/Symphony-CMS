		...
		<ul class="{$class-pagination}">
			...
			<xsl:if test="$show-navigation = true()">
				<li>
					...
					<a class="{$class-previous}" aria-label="{$label-previous}">
						...
						<i class="fas fa-angle-left"></i> <span class="buttontxt"><xsl:value-of select="$label-previous" /></span>
					</a>
				</li>
			</xsl:if>
			...
			<xsl:if test="$show-navigation = true()">
				<li>
					...
					<a class="{$class-next}" aria-label="{$label-next}">
						...
						<span class="buttontxt"><xsl:value-of select="$label-next" /></span> <i class="fas fa-angle-right"></i>
					</a>
				</li>
			</xsl:if>
...
<xsl:template name="pagination-numbers">
	...
	<li class="{$class-page}-container">
		...
		<a class="{$class-page}">
			...
			<xsl:if test="$page = $page-current">
				<span class="current-span"><xsl:value-of select="//data/translation-fields/entry[@id='689']/field-name[@lang=$lang]" /></span>
			</xsl:if>
			<xsl:value-of select="$page" />
			<xsl:if test="$page = $page-current"><span class="current-total">/<xsl:value-of select="$page-total" /></span></xsl:if>
		</a>
	</li>
