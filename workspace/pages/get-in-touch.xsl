...
<xsl:template match="data">
	...
	<xsl:variable name="hasHeadImgClass"><xsl:if test="//data/pages/entry/header-img">has-background</xsl:if></xsl:variable>

	<xsl:variable name="accentHexHashQuery"><xsl:value-of select="substring(//data/pages/entry/accent-hex, 1, 1)" /></xsl:variable>
	<xsl:variable name="accentHex">
		<xsl:choose>
			<xsl:when test="$accentHexHashQuery = '#'"><xsl:value-of select="//data/pages/entry/accent-hex" /></xsl:when>
			<xsl:otherwise><xsl:value-of select="concat('#', //data/pages/entry/accent-hex)" /></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
		...
		<header class="page-header">
			<xsl:choose>
				<xsl:when test="//data/pages/entry/header-img">
					<div class="header-img">
						<xsl:attribute name="style">
							background-image:url(<xsl:value-of select="$workspace"/><xsl:value-of select="//data/pages/entry/header-img/@path"/>/<xsl:value-of select="//data/pages/entry/header-img/filename"/>)
						</xsl:attribute>
					</div>
				</xsl:when>
				<xsl:otherwise>
			...
			<xsl:if test="//data/pages/entry/accent-hex">
				...
				<div class="container">
					<div class="row">
						<div class="col-xs-12 page-title">
							<h1>
								<xsl:value-of select="//data/pages/entry/title[@lang=$lang]" />
							</h1>
						</div>
						<xsl:if test="//data/pages/entry/blurb">
							<div class="col-xs-12 col-sm-9 col-md-8 col-lg-6 page-blurb">
								<xsl:copy-of select="//data/pages/entry/blurb[@lang=$lang]/*" />
							</div>
						</xsl:if>
					</div>
				</div>
